...
init python: 
    ...
    class Item(store.object):
        def __init__(self, name, desc, icon=False, value=0, weight=0, act=Show("inventory_popup", message="Nothing happened!"), type="item", recipe=False):
            global cookbook
            self.name = name
            self.desc = desc
            self.icon = icon
...
screen inventory_screen(first_inventory, second_inventory=False, trade_mode=False, bank_mode=False):
    ...
    imagemap:
        ground "gui/invpanel.png"
    
    key "K_ESCAPE" action Hide("inventory_screen")
    
    key "K_UP" action [None]
    key "K_RIGHT" action [None]
    key "K_DOWN" action [None]
    key "K_LEFT" action [None]
        ...
        hbox:
            ...
            vbox:
                ...
                use sort_nav(first_inventory)
                    ...
                    #textbutton "Крафт" action ToggleScreenVariable("crafting_screen")
                    
                textbutton "Закрыть" action Hide("inventory_screen")
            if second_inventory:
                vbox:
                    label second_inventory.name  
                    use money(second_inventory, first_inventory, bank_mode) #use money(second_inventory, first_inventory, bank_mode)                      
...
screen crafting(inventory):
    vbox:            
        ...
        hbox:
            ...
            text "Предмет" xalign 0.5   
            text "Ингридиенты" xalign 0.5   
        side "c r":
            area (0,0,575,240)
        ...
        #textbutton "Скрыть" action ToggleScreenVariable("crafting_screen") xalign 0.5
...
screen sort_nav(inventory):
    hbox:
        text "Сорт: "         
        textbutton "Имени" action [ToggleField(inventory, "sort_by", inventory.sort_name), inventory.sort_name]
        textbutton "Кол-ву" action [ToggleField(inventory, "sort_by", inventory.sort_qty), inventory.sort_qty]
        textbutton "Цене" action [ToggleField(inventory, "sort_by", inventory.sort_value), inventory.sort_value]
